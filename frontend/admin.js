(function () {
  ...
  function attackFlowHtml(team) {
    ...
    return '<div class="flow-match-stage"><span>当前流程</span><b>' + esc(phases[sidePhase || match.phase] || sidePhase || match.phase) + prophetClock + '</b><small>双方并行攻擂 · 比分 ' + (match.winsA || 0) + ' : ' + (match.winsB || 0) + '</small></div>' +
      ...
      '<div class="flow-attack-total"><span>积累攻击 ' + Number(team.accumulationPoints || 0) + '</span><strong>' + (roll && roll.finalAttack != null ? '本轮总攻击力 ' + Number(roll.finalAttack).toFixed(2) : '系数 ×' + Number(team.growthCoefficient || 1).toFixed(4)) + '</strong></div>';
  }
