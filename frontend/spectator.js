...
function rollHtml(match,side){var roll=match.rolls&&match.rolls[side];if(!roll)return'<span class="feed-empty">等待投骰</span>';var attack=roll.finalAttack==null?roll.attack:roll.finalAttack;return'<span class="feed-dice">'+(roll.dice||[]).map(function(v){return'<i>'+v+'</i>';}).join('')+'</span><small>攻击 '+attack+(roll.syncOk?' · 同步加成':'')+(roll.sandboxAssisted?' · 沙盘助攻':'')+'</small>';}
