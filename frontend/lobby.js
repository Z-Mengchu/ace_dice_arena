...
function connect(){if(es)es.close();es=new EventSource('/api/lobby/events');es.onopen=function(){queueRefresh('lobby');queueRefresh('game');};es.onmessage=function(e){var m=JSON.parse(e.data);if(m.type==='lobby')queueRefresh('lobby');if(m.type==='game')queueRefresh('game');if(m.type==='chat')appendChat('<div class="chat-msg"><b>'+esc(m.sender)+'</b><p>'+esc(m.content)+'</p></div>');if(m.type&&m.type.indexOf('feed:')===0)appendFeed(m);};}
