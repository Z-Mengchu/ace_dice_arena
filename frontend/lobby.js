...
/** 猜阵揭晓弹幕：频道展开时本人该局提交过猜阵才播；收起时直接播；按本方猜中人次分档，同一 match+round 只播一次 */
function commentLobbyGuess(match,entry,side){try{if(!match||!entry)return;var key=match.id+':'+entry.round;if(!chatFeedRelaxed()&&!guessSubmittedLog[key])return;var hits=Number(entry['guessHits'+side]);if(!isFinite(hits))return;var myName=data&&data.me?data.me.displayName:'';if(hits>=2)feedOnce('guess:'+key,'guess-many',myName,'对手套路全被看透！');else feedOnce('guess:'+key,'guess-few',myName,'猜了个寂寞');}catch(e){}}
