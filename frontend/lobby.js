...
function feedMe(mine){var myId=myPlayerId();return (mine.players||[]).find(function(p){return p.id===myId;})||null;}
/* 弹幕门控口径：频道展开时播报已进聊天列表，气泡只保留"与我相关"的留痕；
   频道收起时气泡是聊天列表的替身，放宽为全队可见；单批超过 FEED_BURST_LIMIT 条时合并成一条汇总，避免刷屏 */
var FEED_BURST_LIMIT=5;
function chatFeedRelaxed(){return chatCollapsed;}
/** 掷骰弹幕：频道展开时我本人手动掷过才播（代掷/未参与不弹）；收起时播全队，超量合并汇总 */
function feedLobbyRolls(gs,mine){try{var relaxed=chatFeedRelaxed(),me=feedMe(mine);if(!relaxed&&(!me||me.dice==null||me.autoRolled))return;var mk=feedMatchKey(gs,mine),fresh=[];(mine.players||[]).forEach(function(p){var d=Number(p.dice);if(!isFinite(d))return;var fp='roll:'+mk+':'+p.id;if(cmtSeen[fp])return;fresh.push({fp:fp,p:p,big:d>=4});});if(!fresh.length)return;if(relaxed&&fresh.length>FEED_BURST_LIMIT){var big=fresh.filter(function(i){return i.big;}).length,small=fresh.length-big;fresh.forEach(function(i){markCmtSeen(i.fp);});var text=(big?big+' 人手气爆棚':'')+(big&&small?'，':'')+(small?small+' 人的骰子偷偷摆烂了':'');feedOnce('rollBurst:'+mk+':'+fresh.length+':'+big,big>=small?'roll-big':'roll-small','本队播报',text+'！');}else fresh.forEach(function(i){feedOnce(i.fp,i.big?'roll-big':'roll-small',i.p.name,i.big?'手气爆棚，骰子给足排面！':'骰子偷偷摆烂了');});}catch(e){}}
/** 盲盒弹幕：频道展开时我本人开过盒才播（未开不弹）；收起时播全队；档位 0 不播 */
function feedLobbyBoxes(gs,mine){try{var relaxed=chatFeedRelaxed(),me=feedMe(mine);if(!relaxed){var meOpened=me&&(me.blindBoxOpened||me.blindBox!=null||myLobbyBox!=null);if(!meOpened)return;}var mk=feedMatchKey(gs,mine),fresh=[];(mine.players||[]).forEach(function(p){var box=Number(p.blindBox);if(!isFinite(box)||box===0)return;var fp='box:'+mk+':'+p.id;if(cmtSeen[fp])return;fresh.push({fp:fp,p:p,buff:box>0});});if(!fresh.length)return;if(relaxed&&fresh.length>FEED_BURST_LIMIT){var buff=fresh.filter(function(i){return i.buff;}).length,debuff=fresh.length-buff;fresh.forEach(function(i){markCmtSeen(i.fp);});var text=(buff?buff+' 人欧气附体':'')+(buff&&debuff?'，':'')+(debuff?debuff+' 人非酋报到':'');feedOnce('boxBurst:'+mk+':'+fresh.length+':'+buff,buff>=debuff?'box-buff':'box-debuff','本队播报',text+'！');}else fresh.forEach(function(i){feedOnce(i.fp,i.buff?'box-buff':'box-debuff',i.p.name,i.buff?'欧气直接砸脸上！':'非酋 buff 已签收');});}catch(e){}}
/** 重掷弹幕：频道展开时队长（执行者）播全队新条目、队员只播与自己相关的；收起时播全队；首次只见基线，不补播历史记录 */
var lobbyRerollBaseline=-1;
function commentLobbyReroll(gs,mine){try{var log=(mine&&mine.rerollLog)||[];if(lobbyRerollBaseline<0||log.length<lobbyRerollBaseline){lobbyRerollBaseline=log.length;return;}var relaxed=chatFeedRelaxed(),mk=feedMatchKey(gs,mine),myId=myPlayerId(),isCaptain=!!(mine.roles&&mine.roles.captain===myId);for(var i=lobbyRerollBaseline;i<log.length;i++){var item=log[i],from=Number(item.from),to=Number(item.to);if(!isFinite(from)||!isFinite(to)||from===to)continue;if(!relaxed&&!isCaptain&&String(item.playerId)!==String(myId))continue;if(to>from)feedOnce('reroll:'+mk+':'+i,'reroll-up',item.playerName,'队长这波赌对啦！');else feedOnce('reroll:'+mk+':'+i,'reroll-down',item.playerName,'这下血压上来咯');}lobbyRerollBaseline=log.length;}catch(e){}}
/** 猜阵揭晓弹幕：本人该局提交过猜阵才播（未提交不弹，收起频道也不放宽——文案点评的是自己的猜阵战绩），按本方猜中人次分档，同一 match+round 只播一次 */
function commentLobbyGuess(match,entry,side){try{if(!match||!entry)return;var key=match.id+':'+entry.round;if(!guessSubmittedLog[key])return;var hits=Number(entry['guessHits'+side]);if(!isFinite(hits))return;var myName=data&&data.me?data.me.displayName:'';if(hits>=2)feedOnce('guess:'+key,'guess-many',myName,'对手套路全被看透！');else feedOnce('guess:'+key,'guess-few',myName,'猜了个寂寞');}catch(e){}}
...
function syncCaptainUi(gs){
  ...
  box.innerHTML='<div><small>TEAM CAPTAIN</small><b>'+(isSelf?icon('crown')+' 恭喜！你成为本队队长':'本队队长 · '+esc(playerName(mine,captainId)))+'</b><p>'+(isSelf?'分队、重掷、出场顺序都归你掌控，记得盯紧下方操作面板！队内可以先唠唠战术想法~':'分队、重掷、出场顺序由队长统一操作，宝子们可以去队内频道脑洞碰撞 '+icon('chat',16))+'</p></div>';
}
...
function arenaStatusLine(match){
  ...
  if(match.phase==='RESULT')return'展示战报';
  if(match.phase==='OVERTIME_PENDING')return'三项全平等待加赛';
  if(match.phase==='BATTLE')return'第 '+Number(match.round||1)+' / 6 局 · '+(match.roundPhase==='REVEAL'?'结果揭晓':'猜阵进行中');
  return'等待开赛';
}
function renderHero(){
  ...
  else if(data.phase==='PLAYING')desc=icon('swords',16)+' 比赛进行中 · 最新阶段看右侧面板，操作入口在擂台下方';
...
/* 各面板不再就地渲染倒计时（统一由右侧阶段面板展示），voteCountdown 只负责把截止时刻登记给 StagePanel 驱动逻辑 */
function voteCountdown(deadline,label){if(deadline!=null&&isFinite(Number(deadline))){floatDeadline=Number(deadline);floatLabel=label||'';}return'';}
var lastExpireRefreshAt=0;
function updateVoteCountdowns(){var now=Date.now(),floatDl=updateFloatCountdown();if(floatDl&&now>floatDl&&now-lastExpireRefreshAt>1500){lastExpireRefreshAt=now;queueRefresh('game');}}
  ...
  if(floatStage==='BATTLE'&&data&&data.me&&gsCache){
    ...
    if(match){
      ...
      if(ts!=null&&isFinite(Number(ts))){
        deadline=Number(ts);
        extra=match.phase==='RESULT'?'本场比赛结果展示中':'第 '+Number(match.round||1)+' 局 · '+(match.roundPhase==='REVEAL'?'结果揭晓':'猜阵 ing');
      }
    }
  }
  if(floatStage==='TACTICS'&&data&&data.me&&gsCache){
    var tTeam=findMine(gsCache),cap=tTeam&&tTeam.roles&&tTeam.roles.captain;
    extra='队长 '+playerName(tTeam,cap)+' 正在重掷骰子，上演田忌赛马';
  }
...
function stageNameOf(stage){return{CAPTAIN_VOTE:'队长投票',SQUAD_FORM:'小队组建',ROLL:'全员掷骰',BLIND_BOX:'盲盒抽 buff',TACTICS:'战术排兵',BATTLE:'对局 PK',RESULT:'结果公布'}[stage]||'阶段倒计时';}
...
function eliminatedPanelHtml(gs,match,mine){var a=gs.teams.find(function(t){return t.id===match.a;})||{id:match.a,name:match.a},b=gs.teams.find(function(t){return t.id===match.b;})||{id:match.b,name:match.b},opponent=match.a===mine.id?b:a,stage=bracketStage(match),myScore=Number(match.a===mine.id?match.winsA:match.winsB)||0,opponentScore=Number(match.a===mine.id?match.winsB:match.winsA)||0,tieText=TournamentUI.tieBreakText(match,match.a===mine.id?'我方':'对方',match.a===mine.id?'对方':'我方');return'<div class="post-match-state is-eliminated"><div class="post-match-signal">✕</div><p class="hub-eyebrow">ELIMINATED · SPECTATOR MODE</p><h2>本场遗憾落败，本队淘汰出局</h2><strong>'+esc(stage.label)+' · 最终比分 '+myScore+' : '+opponentScore+' · 对手「'+esc(opponent.name)+'」晋级</strong>'+(tieText?'<p>'+esc(tieText)+'</p>':'')+'<p>剩下赛程化身观众 '+icon('eye',16)+'：查看实时对战，队内聊天频道依旧可用；明天赛事可以重新冲！</p></div>'+roundsSectionHtml(match,a,b);}
function postMatchHtml(gs,match,mine){var won=match.winner===mine.id,champion=gs.champion,stage=bracketStage(match),sameStage=bracketMatches(gs).filter(function(item){return bracketStage(item).rank===stage.rank;}),finished=sameStage.filter(function(item){return item.status==='done';}).length,next=stage.rank===1?'半决赛':stage.rank===2?'总决赛':'本日比赛结束',opponent=gs.teams.find(function(team){return team.id===(match.a===mine.id?match.b:match.a);}),myScore=match.a===mine.id?match.winsA:match.winsB,opponentScore=match.a===mine.id?match.winsB:match.winsA,tieText=TournamentUI.tieBreakText(match,match.a===mine.id?'我方':'对方',match.a===mine.id?'对方':'我方');
  ...
  if(champion)return'';
  if(won){return'<div class="post-match-state is-waiting"><div class="post-match-signal"><i></i></div><p class="hub-eyebrow">QUALIFIED · WAITING SIGNAL</p><h2>'+icon('check')+' 本场胜利，蹲其他场次出结果！</h2><strong>'+esc(mine.name)+' '+myScore+' : '+opponentScore+' '+esc(opponent?opponent.name:'对手')+'</strong>'+(tieText?'<p>'+esc(tieText)+'</p>':'')+'<p>成功挺进'+next+'！当前 '+esc(stage.shortLabel)+' '+finished+' / '+sameStage.length+' 场结束，全部对战结束系统自动生成下一轮对阵，不用刷新页面。</p><div class="post-match-progress"><i style="width:'+Math.round(finished/Math.max(1,sameStage.length)*100)+'%"></i></div><small>等待的时候可以队内频道唠嗑，下方看别的队伍实时战况。</small></div>';}
  return'<div class="post-match-state is-eliminated"><div class="post-match-signal">✕</div><p class="hub-eyebrow">ELIMINATED · SPECTATOR MODE</p><h2>本场遗憾落败，本队淘汰出局</h2><strong>最终比分 '+myScore+' : '+opponentScore+'</strong>'+(tieText?'<p>'+esc(tieText)+'</p>':'')+'<p>对手「'+esc(opponent?opponent.name:'队伍')+'」晋级 · 剩下赛程化身观众 '+icon('eye',16)+'：查看实时对战，队内聊天频道依旧可用；明天赛事可以重新冲！</p></div>';}
/* ---------- 第一阶段：队长投票 ---------- */
function captainVotePanel(box,gs,mine){
  ...
  if(captain){var waitDeadline=0;(gs.teams||[]).forEach(function(t){if(!(t.roles&&t.roles.captain))waitDeadline=Math.max(waitDeadline,Number(t.roleVoteDeadlineAt)||0);});box.innerHTML='<div class="role-ribbon">'+icon('check')+' 队长投票 · 已完成'+(waitDeadline?' '+voteCountdown(waitDeadline,'等待其他队伍投票'):'')+'</div><h2>咱们队的队长新鲜出炉</h2><div class="captain-elect"><small>TEAM CAPTAIN</small><b>'+esc(playerName(mine,captain))+'</b><span>等全部队伍投票结束，队长就要负责分队 + 战术操作咯。</span></div>';return;}
  var progress='<div class="vote-progress"><b>'+voted+' / '+voters.length+'</b><span>已投票（托管队员不计入）</span></div>';
  if(votes[voterId]){box.innerHTML='<div class="role-ribbon">'+icon('vote')+' 队长投票 · 等待全员提交'+voteCountdown(mine.roleVoteDeadlineAt,'队长投票')+'</div>'+progress+'<h2>'+icon('check')+' 你的队长选票已经提交啦</h2><p>投票全程保密，所有人投完 / 倒计时结束，立刻公布队长人选。</p>';return;}
  if(!voters.some(function(p){return p.id===captainVotePick;}))captainVotePick='';
  box.innerHTML='<div class="role-ribbon">'+icon('vote')+' 第一阶段 · 队长投票 '+voteCountdown(mine.roleVoteDeadlineAt,'队长投票')+'</div>'+progress+'<div class="hub-section-title"><div><small>CAPTAIN VOTE · 每人 1 票哦</small><h2>队内 pick 咱们的总指挥队长</h2></div></div><div class="action-player-grid">'+voters.map(function(p){var selected=p.id===captainVotePick;return'<button class="action-player'+(selected?' selected':'')+'" data-vote-player="'+esc(p.id)+'" aria-pressed="'+selected+'"><b>'+esc(p.name)+'</b><small>'+(p.role==='back'?'后端':'前端')+'</small></button>';}).join('')+'</div><div class="action-footer"><button id="submit-captain-vote" class="btn btn-primary" '+(captainVotePick?'':'disabled')+'>提交队长选票</button></div>';
...
function captainSquadViewHtml(mine){
  ...
  return'<details class="captain-squad-view"'+(squadViewOpen?' open':'')+'><summary><b>'+icon('squad',16)+' 本队阵容</b><span>'+(squadViewOpen?'点击收起':'点击展开')+'</span></summary>'+squadsReadonlyHtml(mine,true)+'</details>';
}
...
function squadFormPanel(box,gs,mine){
  ...
  if(mine.squads){box.innerHTML='<div class="role-ribbon">'+icon('check')+' 第二阶段 · 分队完成 '+voteCountdown(gs.stageDeadlineAt,'队长分队')+'</div><h2>咱们的 6 支小队就位啦</h2>'+squadsReadonlyHtml(mine,false)+'<p>掷骰 + 盲盒结束，队长可以调整 1~6 号小队出战顺序！田忌赛马的精髓就在这里～</p>';return;}
  if(!isCaptain){box.innerHTML='<div class="role-ribbon">'+icon('squad')+' 第二阶段 · 队长分队 '+voteCountdown(gs.stageDeadlineAt,'队长分队')+'</div><h2>坐等队长排兵布阵中 '+icon('loading',16)+'</h2><p>队长 <b>'+esc(playerName(mine,captain))+'</b> 正在把 30 人拆成 6 支 5 人小队。全部队伍搞定后，直接进入掷骰环节！</p>';return;}
  ...
  function draw(){
    ...
    box.innerHTML='<div class="role-ribbon">'+icon('squad')+' 第二阶段 · 队长分队 '+voteCountdown(gs.stageDeadlineAt,'队长分队')+'</div>'
      +'<div class="hub-section-title"><div><small>SQUAD FORM · 6 队 × 5 人</small><h2>把 30 位小伙伴分到 6 个小队！</h2></div><span>'+st.filled+' / '+players.length+(complete?' · 已完成':'')+'</span></div>'
      +'<div class="squad-form-panel">'
      +'<div class="squad-toolbar"><button type="button" id="auto-squad-form" class="btn btn-ghost squad-auto-btn">'+(complete?icon('dice',16)+' 重新随机分队':icon('dice',16)+' 一键随机分队')+'</button><p class="squad-toolbar-tip">偷懒小技巧：先随机再微调～按住拖拽队员换小队，拖回待分配区就移出；小队之间也能互换；点选队员再点小队也可以完成分配。</p></div>'
      +'<div class="squad-pool" data-squad-pool>'+(pool.length?pool.map(poolCardHtml).join(''):'<span class="squad-pool-empty">'+icon('check',16)+' '+players.length+' 名队员分配完毕，还能继续拖拽调整</span>')+'</div>'
    ...
    document.getElementById('auto-squad-form').onclick=function(){var s=squadState();if(s.complete&&!window.confirm('重新随机会全部打乱现有分组，确定要重置吗？'))return;autoFill(s.complete);pick='';draw();};
...
function rollPanel(box,gs,mine){
  ...
  if(myRolled||myLobbyDie!=null||now>closeAt){
    statusHtml=(myDie!=null&&window.DiceRollUI?'<div class="pl-reveal-dice"><div class="pl-reveal-slot mine">'+window.DiceRollUI.dieHTML('big',myDie)+'</div></div>':'')
      +'<p>'+icon('check',16)+' 已经扔完，坐等开盲盒惊喜～</p>';
  }else if(now<openAt){
    statusHtml='<div class="battle-countdown"><small>马上就要摇骰子啦！</small><strong>'+Math.ceil((openAt-now)/1000)+'</strong><p>秒后开掷，时间到直接点按钮就 ok。</p></div>';
  }else{
    statusHtml='<div class="battle-countdown"><small>掷骰进行中 '+icon('dice',16)+'</small><strong>'+Math.max(0,Math.ceil((closeAt-now)/1000))+'</strong><p>窗口剩余秒数，快点击下方按钮掷骰子！</p></div>'
      +'<div class="action-footer"><button id="lobby-roll-btn" class="btn btn-primary btn-xl"'+(lobbyRolling?' disabled':'')+'>掷！</button></div>';
  }
  box.innerHTML='<div class="role-ribbon">'+icon('dice')+' 第三阶段 · 全员掷骰 '+voteCountdown(gs.stageDeadlineAt,'全员掷骰')+'</div>'
    +'<div class="hub-section-title"><div><small>ROLL · 统一截止时间！</small><h2>'+(now<openAt?'马上就要摇骰子啦！':'掷骰进行中 '+icon('dice',16))+'</h2></div><span>'+rolled+' / '+players.length+'</span></div>'
    +statusHtml;
...
function blindBoxPanel(box,gs,mine){
  ...
  else{mineHtml='<p>三选一开启盲盒！档位完全随机，有加分惊喜也有减益，每人仅有一次机会；25 秒不点 = 放弃，按 0 计算，<b>系统不会帮你开盒！</b></p>'+window.BlindBoxUI.boxesHTML();}
  box.innerHTML='<div class="role-ribbon">'+icon('gift')+' 第四阶段 · 盲盒 '+voteCountdown(gs.stageDeadlineAt,'开盲盒')+'</div><div class="hub-section-title"><div><small>BLIND BOX · 限时 25 秒！</small><h2>全员拆盲盒时间</h2></div><span>'+opened+' / '+players.length+'</span></div>'+mineHtml;
...
function tacticsPanel(box,gs,mine){
  ...
  if(!isCaptain){
    box.innerHTML='<div class="role-ribbon">'+icon('brain')+' 第五阶段 · 战术调整 '+voteCountdown(gs.stageDeadlineAt,'战术调整')+'</div>'
      +'<div class="hub-section-title"><div><small>TACTICS · 队长正在疯狂运筹帷幄</small><h2>等待队长完成重掷和排阵</h2></div><span>'+(confirmed?icon('check',16)+' 队长确认完毕，坐等开战':(locked?'出场顺序已锁定':'出场顺序待锁定'))+'</span></div>'
      +'<p>队长 <b>'+esc(playerName(mine,captain))+'</b> 正在操作（重掷 '+used+' / '+limit+' 次）。'+(confirmed?'全部队伍确认就开启对局！':'')+'</p>'+rerollLogHtml(mine)+squadsReadonlyHtml(mine,true);
    return;
  }
  ...
  function draw(){
    ...
    box.innerHTML='<div class="role-ribbon">'+icon('brain')+' 第五阶段 · 战术调整 '+voteCountdown(gs.stageDeadlineAt,'战术调整')+'</div>'
      +'<div class="hub-section-title"><div><small>REROLL · 全队最多 '+limit+' 次重掷机会</small><h2>队长重掷时刻</h2></div><span>剩余 '+left+' 次</span></div>'
      ...
      +rerollLogHtml(mine)
      +'<div class="hub-section-title"><div><small>SQUAD ORDER · 1 号小队最先冲场！</small><h2>出场顺序</h2></div><span>'+(locked?icon('lock',16)+' 已锁定':'未锁定')+'</span></div>'
      +(mine.squads?'<div class="squad-order-list">'+order.map(function(origNo,pos){return orderCardHtml(origNo,pos);}).join('')+'</div>':'<p>分队数据还在加载，请稍候。</p>')
      +(locked?'<p class="squad-order-locked-note">出场顺序已经锁死，对局就按这个顺序开打。</p>':'<div class="action-footer"><button id="lock-squad-order" class="btn btn-primary" '+(mine.squads&&!confirmed?'':'disabled')+'>锁定出场顺序</button></div>')
      +'<div class="tactics-confirm-bar">'+(confirmed
        ?'<span class="tactics-confirm-state is-done">'+icon('check',16)+' 战术布置搞定！重掷 &amp; 排序已锁定</span><button id="tactics-cancel-confirm" class="btn btn-ghost">取消确认</button>'
        :'<span class="tactics-confirm-state">确认之后重掷和排阵全部锁定；全部队伍就绪马上开战 '+icon('swords',16)+'</span><button id="tactics-confirm" class="btn btn-primary">完成战术布置</button>')+'</div>';
    box.querySelectorAll('[data-reroll]').forEach(function(btn){btn.onclick=function(){var p=players.find(function(x){return x.id===btn.dataset.reroll;});if(!window.confirm('确定给 '+(p?p.name:'该队员')+' 重新掷骰子吗？重掷之后骰子点数会随机刷新哦！'))return;rerollBusy=true;btn.disabled=true;playerAction('reroll',[btn.dataset.reroll]).then(function(){rerollBusy=false;}).catch(function(error){rerollBusy=false;showActionError(error);});};});
    ...
    if(lock)lock.onclick=function(){if(!window.confirm('锁定之后再也不能调整出场顺序啦，确认提交？'))return;lock.disabled=true;playerAction('squad-order',squadOrderDraft.map(function(n){return String(n);})).then(function(){squadOrderDraft=null;}).catch(function(error){lock.disabled=false;showActionError(error);});};
...
function roundEntryHtml(entry,a,b){var w=entry.winner,winnerName=w?(w==='A'?a:b).name:'';return'<div class="round-entry"><div class="round-entry-head"><b>第 '+Number(entry.round)+' 局</b><span>'+(w?esc(winnerName)+' 队 胜利':'平局')+'</span></div><div class="round-entry-sides">'+roundSideHtml(a.name,entry,'A',w==='A')+roundSideHtml(b.name,entry,'B',w==='B')+'</div></div>';}
...
function roundsSectionHtml(match,a,b){var rounds=match.rounds||[];if(!rounds.length)return'';if(expandedRounds[match.id]){var detail=detailFor(match);return'<div class="rounds-toggle"><button type="button" class="btn btn-ghost rounds-toggle-btn" data-rounds-match="'+esc(match.id)+'">收起战报</button><div class="rounds-detail">'+(detail?roundsListHtml(detail,a,b):'<p class="rounds-loading">战况加载中…</p>')+'</div></div>';}return'<div class="rounds-toggle"><button type="button" class="btn btn-ghost rounds-toggle-btn" data-rounds-match="'+esc(match.id)+'">查看完整对局战报（'+rounds.length+' 局）</button></div>';}
...
function preGuessHtml(match,mine,a,b,side){
  ...
  if(submitted&&!preGuessEdit)return'<div class="pl-preguess"><div class="hub-section-title"><div><small>PRE-GUESS · 后面第 '+myRound+' 局你会上场 '+icon('sparkle',16)+'</small><h2>'+icon('check')+' 提前猜阵提交成功</h2></div></div><p>第 '+myRound+' 局开打自动生效，结果揭晓前随时撤回 / 改选！</p><div class="action-footer"><button type="button" class="btn btn-ghost" id="preguess-retract">撤回提前猜阵</button><button type="button" class="btn btn-primary" id="preguess-edit">改投</button></div></div>';
  ...
  return'<div class="pl-preguess"><div class="hub-section-title"><div><small>PRE-GUESS · 后面第 '+myRound+' 局你会上场 '+icon('sparkle',16)+'</small><h2>提前预判对手阵容！</h2></div></div><p>趁排队，猜敌方第 '+myRound+' 局派出哪 5 位队员；猜中加战力！对局自动生效，揭晓前可以撤回重选。</p><div class="pl-roster">'+grid+'</div><div class="action-footer"><span id="preguess-n">已选 '+preGuessSel.length+' / 5</span><button type="button" class="btn btn-primary" id="preguess-submit"'+(preGuessSel.length===5?'':' disabled')+'>提前提交猜阵</button></div></div>';
}
function bindPreGuess(box,match,mine){
  ...
  if(submit)submit.onclick=function(){if(preGuessSel.length!==5)return;submit.disabled=true;playerAction('pre-guess',preGuessSel.slice()).then(function(){preGuessEdit=false;markGuessSubmitted(match.id+':'+myRound);}).catch(function(error){submit.disabled=false;showActionError(error);});};
...
function battlePanel(box,gs,mine){
  ...
  if(match.phase==='OVERTIME_PENDING'){box.innerHTML=head+'<div class="match-result-banner"><small>OVERTIME</small><h2>'+icon('warn')+' 三项全部打平｜等待加赛！</h2><strong>6 局对战比分 '+Number(match.winsA||0)+' : '+Number(match.winsB||0)+'</strong><p>胜场、总点数、GMV 全部持平，等待管理员安排加赛一决高下。</p></div>'+roundsSectionHtml(match,a,b);bindRoundsSection(box,match,a,b);return;}
  if(match.phase!=='BATTLE'){box.innerHTML=head+'<p>对阵还没开打，等待系统开局~</p>';return;}
  ...
  if(match.roundPhase==='REVEAL'){
    ...
    if(last)commentLobbyGuess(match,last,side);
    box.innerHTML=head+'<div class="guess-panel is-reveal"><div class="hub-section-title"><div><small>ROUND '+Number(match.round)+' / 6 · REVEAL</small><h2>第 '+Number(match.round)+' 局结果大揭晓</h2></div></div>'+(last?roundEntryHtml(last,a,b)+'<p>冲向下一局！</p>':'<p>本局结果加载中…</p>')+'</div>'+preGuessHtml(match,mine,a,b,side)+roundsSectionHtml(match,a,b);
  ...
  box.innerHTML=head+'<div class="guess-panel"><div class="hub-section-title"><div><small>ROUND '+Number(match.round)+' / 6 · GUESS</small><h2>'+icon('crystal')+' 第 '+Number(match.round)+' 局 · 猜阵进行中</h2></div>'+voteCountdown(match.guessDeadlineAt,'第 '+Number(match.round)+' 局 · 猜阵')+'</div><p>本局双方 '+Number(match.round)+' 号小队 battle！出战宝子猜猜对面会上哪 5 个人，猜中直接给小队加战力！</p><div class="guess-status"><span>我方出战小队已提交 <b>'+mySubmitted+' / 5</b></span><span>对方已提交 <b>'+oppSubmitted+' / 5</b></span></div>'+(inSquad?'<div class="action-footer"><a class="btn btn-primary" href="/player">前往提交猜阵</a></div>':(pre||'<p class="timing-spectator-note">你不在本局出战，安心看大家提交进度就好。</p>'))+'</div>'+roundsSectionHtml(match,a,b);
  bindRoundsSection(box,match,a,b);
  bindPreGuess(box,match,mine);
}
...
function championPersonalHtml(championId,overall){var me=data&&data.me;if(!me||!me.teamId)return'';if(me.teamId===championId)return'<p class="champion-personal is-self">🎉 恭喜！咱们队伍夺冠！</p>';return'<p class="champion-personal">'+(overall?'两天全部赛程结束，最终战绩已保存。':'今日赛程结束，战绩存档。')+'</p>';}
function renderChampionAnnouncement(gs){var box=document.getElementById('champion-announcement');if(!box){box=document.createElement('section');box.id='champion-announcement';box.className='hub-card champion-announcement hidden';var hero=document.getElementById('lobby-hero');hero.parentNode.insertBefore(box,hero.nextSibling);}/* 冠军卡只在非比赛进行阶段展示：当日冠军产生时大厅阶段会转为 FINISHED，新一轮比赛一开始（PLAYING）即隐藏，避免把操作面板挤到下方 */if(data&&data.phase==='PLAYING'){box.className='hub-card champion-announcement hidden';box.innerHTML='';return;}var results=gs&&gs.dayResults||{},overall=gs&&gs.overallResult;if(overall&&overall.champion){var decidedText={BOTH_DAYS:'两日双冠',MATCH_WINS:'累计胜场决胜',GMV:'GMV 决胜',OVERTIME:'加赛封神夺冠 🏆'}[overall.decidedBy]||'',winner=(overall.standings||[]).find(function(team){return team.id===overall.champion;})||{},entries=[1,2].map(function(day){var result=results['day'+day],team=result&&(result.teams||[]).find(function(item){return item.id===overall.champion;});return team?{day:day,team:team}:null;}).filter(Boolean);box.className='hub-card champion-announcement is-overall';box.innerHTML='<div class="champion-summary"><div class="champion-crown">🏆</div><div><p class="hub-eyebrow">TWO-DAY GRAND CHAMPION</p><h2>'+icon('trophy')+' 两天总冠军 · '+esc(winner.name||overall.champion)+'</h2><p>两天累计 <b>'+Number(winner.totalMatchWins||0)+'</b> 场胜利'+(decidedText?' · '+decidedText:'')+'</p>'+championPersonalHtml(overall.champion,true)+'</div></div>'+TournamentUI.resultRosterHtml(entries);return;}if(overall&&overall.status==='OVERTIME_PENDING'){var candidates=overall.candidates||[],candidateNames=candidates.map(function(team){return esc(team.name);}).join(' vs ');box.className='hub-card champion-announcement is-overall';box.innerHTML='<div class="champion-summary"><div class="champion-crown">🏆</div><div><p class="hub-eyebrow">TWO-DAY GRAND CHAMPION</p><h2>两天总冠军仍需加赛 · '+candidateNames+'</h2><p>两天胜场 &amp; GMV 全部持平，等待管理员开启总决赛加赛！</p></div></div>';return;}var daily=results.day2||results.day1;if(daily&&daily.champion){var team=(daily.teams||[]).find(function(item){return item.id===daily.champion;})||{};box.className='hub-card champion-announcement is-daily';box.innerHTML='<div class="champion-summary"><div class="champion-crown">🏆</div><div><p class="hub-eyebrow">DAY '+Number(daily.day||1)+' CHAMPION</p><h2>今日冠军 · '+esc(team.name||daily.champion)+'</h2><p>今日总决赛落幕！比赛战绩已保存。</p>'+championPersonalHtml(daily.champion,false)+'</div></div>'+TournamentUI.resultRosterHtml([{day:Number(daily.day||1),team:team}]);return;}box.className='hub-card champion-announcement hidden';box.innerHTML='';}
function loadGameState(){if(gameLoading)return;gameLoading=true;fetch('/api/game-state').then(function(r){return r.status===204?null:r.json();}).then(function(d){var gs=d&&d.state;gsCache=gs;if(gs&&gs.stage==='ROLL')myLobbyBox=null;if(gs&&gs.stage!=='ROLL')myLobbyDie=null;renderChampionAnnouncement(gs);renderHero();renderPlayerActions(gs);}).catch(function(){}).finally(function(){gameLoading=false;});}
...
var FEED_META={'roll-big':['dice',1],'roll-small':['dice',0],'box-buff':['gift',1],'box-debuff':['gift',0],'reroll-up':['sparkle',1],'reroll-down':['warn',0],'guess-many':['crystal',1],'guess-few':['crystal',0]};
...
function connect(){if(es)es.close();es=new EventSource('/api/lobby/events?tab='+sseTabId());es.onopen=function(){queueRefresh('lobby');queueRefresh('game');};es.onmessage=function(e){var m=JSON.parse(e.data);if(m.type==='lobby')queueRefresh('lobby');if(m.type==='game')queueRefresh('game');if(m.type==='chat')appendChat('<div class="chat-msg"><b>'+esc(m.sender)+'</b><p>'+esc(m.content)+'</p></div>');if(m.type&&m.type.indexOf('feed:')===0)appendFeed(m);};}
