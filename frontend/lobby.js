...
function feedOnce(fp,kind,name,text){try{if(!fp||cmtSeen[fp])return;if(!window.Commentary||!window.Commentary.event)return;cmtSeen[fp]=1;try{sessionStorage.setItem(CMT_KEY,JSON.stringify(cmtSeen));}catch(e){}window.Commentary.event({kind:kind,name:name,text:text});}catch(e){}}
...
/** 掷骰弹幕：我本人手动掷过才播（代掷/未参与不弹）；本队成员点数首次出现时逐条播 */
function feedLobbyRolls(gs,mine){try{var me=feedMe(mine);if(!me||me.dice==null||me.autoRolled)return;var mk=feedMatchKey(gs,mine);(mine.players||[]).forEach(function(p){var d=Number(p.dice);if(!isFinite(d))return;if(d>=4)feedOnce('roll:'+mk+':'+p.id,'roll-big',p.name,'手气爆棚，骰子给足排面！');else feedOnce('roll:'+mk+':'+p.id,'roll-small',p.name,'骰子偷偷摆烂了');});}catch(e){}}
/** 盲盒弹幕：我本人开过盒才播（未开不弹）；档位 0 不播 */
function feedLobbyBoxes(gs,mine){try{var me=feedMe(mine);if(!me)return;var meOpened=me.blindBoxOpened||me.blindBox!=null||myLobbyBox!=null;if(!meOpened)return;var mk=feedMatchKey(gs,mine);(mine.players||[]).forEach(function(p){var box=Number(p.blindBox);if(!isFinite(box)||box===0)return;if(box>0)feedOnce('box:'+mk+':'+p.id,'box-buff',p.name,'欧气直接砸脸上！');else feedOnce('box:'+mk+':'+p.id,'box-debuff',p.name,'非酋 buff 已签收');});}catch(e){}}
/** 重掷弹幕：队长（执行者）播全队新条目，队员只播与自己相关的；首次只见基线，不补播历史记录 */
var lobbyRerollBaseline=-1;
function commentLobbyReroll(gs,mine){try{var log=(mine&&mine.rerollLog)||[];if(lobbyRerollBaseline<0||log.length<lobbyRerollBaseline){lobbyRerollBaseline=log.length;return;}var mk=feedMatchKey(gs,mine),myId=myPlayerId(),isCaptain=!!(mine.roles&&mine.roles.captain===myId);for(var i=lobbyRerollBaseline;i<log.length;i++){var item=log[i],from=Number(item.from),to=Number(item.to);if(!isFinite(from)||!isFinite(to)||from===to)continue;if(!isCaptain&&String(item.playerId)!==String(myId))continue;if(to>from)feedOnce('reroll:'+mk+':'+i,'reroll-up',item.playerName,'队长这波赌对啦！');else feedOnce('reroll:'+mk+':'+i,'reroll-down',item.playerName,'这下血压上来咯');}lobbyRerollBaseline=log.length;}catch(e){}}
/** 猜阵揭晓弹幕：本人该局提交过猜阵才播（未提交不弹），按本方猜中人次分档，同一 match+round 只播一次 */
