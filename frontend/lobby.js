...
function commentLobbyBox(boxValue,gs,mine){try{var box=Number(boxValue);if(!isFinite(box)||box===0)return;var match=heroMatchFor(gs,mine&&mine.id);var fp='box:'+(match&&match.id?match.id:String((gs&&gs.stage)||'')+':'+(mine?mine.id:''))+':'+box;if(box>0)cmtOnce(fp,'欧气附体！个人点数蹭蹭上涨～','good');else cmtOnce(fp,'非酋报到，点数直接被砍一刀！','bad');}catch(e){}}
...
function connect(){if(es)es.close();es=new EventSource('/api/lobby/events');es.onopen=function(){queueRefresh('lobby');queueRefresh('game');};es.onmessage=function(e){var m=JSON.parse(e.data);if(m.type==='lobby')queueRefresh('lobby');if(m.type==='game')queueRefresh('game');if(m.type==='chat'){var l=document.getElementById('chat-list');l.insertAdjacentHTML('beforeend','<div class="chat-msg"><b>'+esc(m.sender)+'</b><p>'+esc(m.content)+'</p></div>');l.scrollTop=l.scrollHeight;}};}
