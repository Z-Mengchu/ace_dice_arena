...
(function () {
  ...
  var rules = {
    CAPTAIN_VOTE: {
      step: '第一阶段', title: '队长投票', icon: 'vote', summary: '开战前队内投票 pick 咱们的总指挥！全员每人 1 票，25 秒内完成。分队、排兵布阵全靠靠谱队长带队~',
      items: ['队内所有人都可以参选，投给你信任的小伙伴！', '一人只有 1 票，系统托管的队友不能参与投票。', '票数打平 / 没人投票，直接取名单排序第一位当队长。', '25 秒没投票直接弃权，系统按现有票数统计。']
    },
    SQUAD_FORM: {
      step: '第二阶段', title: '队长分组', icon: 'squad', summary: '队长把 30 人划分 6 支小队，每队 5 人！同时定好 1-6 出场顺位，限时 45 秒。分组搭配很关键，田忌赛马的第一步！',
    ...
    BLIND_BOX: {
      step: '第四阶段', title: '开盲盒抽 buff', icon: 'gift', summary: '掷完骰子拆盲盒！三选一，有加分惊喜也有减益 debuff，直接刷新你的个人点数，限时 10 秒纯拼手气！',
      items: ['3 个盲盒选 1 个开启，开完还可以偷看剩下两个盒子内容。', '点数有加分也有减益，开盒前完全保密，全看运气。', '大概率能拿到加分，但超级大奖极为稀有！锦鲤在哪里！', '10 秒不选择直接放弃，计 0 分，系统不会帮你开盲盒。']
    },
    TACTICS: {
      step: '第五阶段', title: '战术窗口｜重掷 + 排阵', icon: 'brain', summary: '45 秒战术博弈时间！队伍 GMV 兑换重掷次数，队长指定队员重新摇骰子；排好 6 支小队出场顺序。用好机会极限翻盘！',
      items: ['每 10 万 GMV 兑换 1 次重掷，单场最多 5 次，由队长指定重掷队员。', '重掷只刷新骰子点数，掷骰时间、盲盒加成保留，暴击不受干扰。', '出场顺序锁定不可修改！强弱小队怎么对位，考验队长田忌赛马的智慧。', '队长点确认即按当前排阵锁定出场顺序、代表本队就绪；取消确认只能补重掷，顺序不可回头；全部队伍确认立刻开打。', '超时没有操作，剩余重掷次数直接作废，出场顺序默认小队编号锁定。']
    },
    BATTLE: {
      step: '第六阶段', title: '六局对战', icon: 'swords', summary: '开打前秘密猜对手出场阵容，猜中直接涨战力！双方小队按顺序开启 6 场 5v5 PK，小队战力更高拿下单局胜利！',
      items: ['全员 30 秒统一猜阵：每人秘密提交「敌方本人局出战 5 人」的猜测，互相看不到对方猜测。', '窗口内随时可以撤回、改投；双方 30 份全部交齐后 5 秒揭晓（剩余不足 5 秒则按原倒计时）。', '倒计时结束强制揭晓，不等晚到的人；6 局结果一屏同时公布。', '猜中 1 位对手，小队战力 +0.4，单局最多 +10 分上限。', '小队战力 = 5 人点数总和（暴击则 ×1.5）+ 猜阵加成；战力高赢下本局，战力相同则本局双方不得分。', '6 局结果同屏展示后进入结算，一口气看完全场对决。']
    },
    RESULT: {
