...
管理员点击开始后，八支队伍先各自投票选出队长（20 秒，平票按队员名单顺序取先），队长把本队 30 人编成 6 支 5 人小队（120 秒，超时系统随机均分），分队结果全天锁定；随后抽签生成 1/4 决赛 ×4。每个淘汰轮次，所有在赛队伍按同一时间轴推进：全员 321 倒计时后 30 秒内各掷 1 枚骰子 → 每人开盲盒（60 秒）→ 队长重掷与排定 1~6 号出场顺序（120 秒）→ 6 局对局逐局猜阵与揭晓。单局小队战力高者胜，6 局胜场多者晋级；平局依次比总点数、增长系数、队伍 ID。首轮结束后自动生成两场半决赛，再自动生成决赛。主持区是只读的实时赛事墙，可同时查看八队进度与当前全部场次的比分与每轮明细。
